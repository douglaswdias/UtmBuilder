...
public class Utm
{
    ...
    public static implicit operator Utm(string link)
    {
        ...
        var param = segments[1].Split("&");
        var source = param.Where(x => x.StartsWith("utm_source")).FirstOrDefault("").Split("=")[1];
        var medium = param.Where(x => x.StartsWith("utm_medium")).FirstOrDefault("").Split("=")[1];
        var name = param.Where(x => x.StartsWith("utm_campaign")).FirstOrDefault("").Split("=")[1];
        var id = param.Where(x => x.StartsWith("utm_id")).FirstOrDefault("").Split("=")[1];
        var term = param.Where(x => x.StartsWith("utm_term")).FirstOrDefault("").Split("=")[1];
        var content = param.Where(x => x.StartsWith("utm_content")).FirstOrDefault("").Split("=")[1];
